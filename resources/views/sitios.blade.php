	...
	<div id="fh5co-page">
		...
		<div id="fh5co-main">
			...
			<div class="fh5co-narrow-content">
				<h2 class="fh5co-heading animate-box" data-animate-effect="fadeInLeft">Portales Web</h2>
					<div class="row">
  ...
  <div class="navbar-container">
    <div class="navbar" id="navbar">
        @foreach ($enlaces as $id => $enlace)
            <div class="nav-item" onclick="showContent('{{ $id }}')">{{ $enlace['nombre'] }}</div>
        @endforeach
    </div>
</div>

<!-- Contenedor para mostrar información del enlaces seleccionado -->
<div class="content" id="content-box">
    <h2 id="content-title"></h2>
    <ul class="enlaces" id="content-list"></ul>
</div>
...
<script>
    ...
    // Objeto con la información de los enlaces (se llena con Blade)
    const enlacesData = @json($enlaces);

    function showContent(id) {
        let enlaces = enlacesData[id];

        if (enlaces) {
            contentTitle.textContent = enlaces.nombre;
            contentList.innerHTML = "";

            if (enlaces.enlaces.length > 0) {
                enlaces.enlaces.forEach(ext => {
                    let li = document.createElement("li");
                    li.innerHTML = `<a href="${ext.enlace}"><i class="bi bi-link-45deg text-primary"></i>${ext.nombre}</a>`;
                    contentList.appendChild(li);
                });
            } else {
                contentList.innerHTML = "<li>No hay enlaces registrados</li>";
            }
